...
export const SalesAnalyticsPage: React.FC = () => {
    ...
    const loadData = async () => {
      ...
      try {
        const [fetchedSales, fetchedProducts, fetchedCategories, fetchedManufacturers] = await Promise.all([
          ...
          productService.fetchAllProductsAdmin(), // Fetch all products to map IDs to names
  ...
  if (isLoading) {
    return <p className="text-center text-gray-500 py-10">{t('common.loading')} {t('adminPage.tabAnalytics').toLowerCase()}...</p>;
  }
  if (error) {
...
// Add to locales (en.json, sk.json) under an "analytics" key:
...
// "revenueByManufacturer": "Revenue by Manufacturer",
