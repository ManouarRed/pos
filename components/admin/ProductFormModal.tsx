...
import { Product, ProductFormData, Category, Manufacturer } from '../../types';
...
export const ProductFormModal: React.FC<ProductFormModalProps> = ({ isOpen, onClose, onSave, product }) => {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.image.trim()) {
        errors.image = t('common.imageURL') + " " + t('errors.isRequired');
    } else {
        try {
            new URL(formData.image);
        } catch (_) {
            errors.image = t('common.imageURL') + " " + t('errors.isInvalid');
        }
    }
