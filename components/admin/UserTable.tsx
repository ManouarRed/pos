                ...
                {users.map((user) => (
                  <tr key={user.id} className="hover:bg-gray-50 transition-colors duration-150">
                    ...
                    <td className="whitespace-nowrap py-4 pl-3 pr-4 text-center text-sm font-medium sm:pr-6 space-x-1">
                      <Button variant="ghost" size="sm" onClick={() => onEdit(user)} aria-label={`${t('common.edit')} ${user.username}`} className="group inline-flex items-center">
                        <PencilIcon className="text-indigo-600 group-hover:text-indigo-800 h-5 w-5" />
                        <span className="ml-1 text-xs text-indigo-600 group-hover:text-indigo-800">{t('common.edit')}</span>
                      </Button>
                      <Button
                        variant="ghost"
                        size="sm"
                      ...
                      >
                        <TrashIcon className={`h-5 w-5 ${user.id === currentUser.id ? 'text-gray-400' : 'text-red-500 group-hover:text-red-700'}`} />
                        <span className={`ml-1 text-xs ${user.id === currentUser.id ? 'text-gray-400' : 'text-red-500 group-hover:text-red-700'}`}>{t('common.delete')}</span>
                      </Button>
                    </td>
                  </tr>
                ))}
